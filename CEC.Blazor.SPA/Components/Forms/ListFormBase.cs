...
namespace CEC.Blazor.SPA.Components.Forms
{
         ...
         where TContext : DbContext
    {
        ...
        protected async override Task OnRenderAsync(bool firstRender)
        {
            ...
            if (this.IsService)
            {
                if (firstRender)
                {
                    ...
                    this.ListTitle = string.IsNullOrEmpty(this.ListTitle) ? $"List of {this.Service.RecordInfo.RecordListDescription}" : this.ListTitle;

                }
                // Load the filters for the recordset
                this.LoadFilter();
                if (this.IsViewManager && !this.ViewManager.ViewData.ViewFields.TryGet<int>("Page", out page)) page = 1;
        ...
        {
            if (this.IsViewManager) this.ViewManager.ViewData.ViewFields.Set("Page", page);
