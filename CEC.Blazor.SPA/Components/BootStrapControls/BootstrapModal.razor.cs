...
namespace CEC.Blazor.SPA.Components.UIControls
{
    ...
    public partial class BootstrapModal : Component, IModal
    {
        ...
        private string _ContainerCss => $"modal fade show {this.Options.Parameters.Get<string>("ContainerCSS")}".Trim();

        private string _ModalCss => $"modal-dialog {this.Options.Parameters.Get<string>("ModalCSS")}".Trim();

        private string _ModalHeaderCss => $"modal-header {this.Options.Parameters.Get<string>("ModalHeaderCSS")}".Trim();
